...
import (
	...
	"github.com/bartek5186/procyon/internal/templateinit"
)
...
func main() {
	...
	switch os.Args[1] {
	case "init":
		initCmd := flag.NewFlagSet("procyon init", flag.ExitOnError)
		...
		if err := templateinit.Run(opts); err != nil {
			fmt.Fprintf(os.Stderr, "procyon init: %v\n", err)
			os.Exit(1)
		}
	default:
		usage()
...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "  procyon init\n")
	fmt.Fprintf(os.Stderr, "  procyon init --name przyjazne-server --module github.com/acme/przyjazne-server --db postgres --auth kratos-casbin --out ../przyjazne-v2\n")
}
