...
RBAC nie ufa rolom z Kratos `traits`, `metadata_public` ani `metadata_admin`. Kratos dostarcza tylko identity ID, a role i polityki są rozwiązywane przez Casbin z app DB.

`rbac.default_role` jest nadawane nowej zalogowanej identity, jeśli nie ma jeszcze roli w Casbinie. `rbac.admin_identity_ids` służy do bootstrapu początkowych adminów.
...
Jeśli feature dodaje tabelę, dodaj model do `runAutoMigrate` i przygotuj migrację SQL `goose` w `internal/migrations/<driver>/`, jeśli projekt używa już wersjonowanych migracji. Domyślnie template używa `database.auto_migrate=true`; przejście na goose wymaga ustawienia `database.auto_migrate=false`.

Nie traktuj `AutoMigrate` jako docelowego systemu migracji dla produkcyjnych danych. Główny problem nie polega na tym, że usuwa dane, tylko na braku jawnej historii zmian, rollbacków, statusu wersji schematu i kontroli nad zmianami wymagającymi ręcznego SQL.
